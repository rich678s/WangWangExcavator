    ...
    for key_word in KEYWORD:
        for link in a_soup:
            if key_word in link.text:
                result_dict[link.text.strip()] = sillyURLReformat(website, link['href'])
    return result_dict
...
def main():
    ...
    for website, a_soup in all_link_in_soup.items():
        ...
        logger.debug("Parsed %s, Matched %d links." % (website, len(title_and_link)))
    ...
    for website, title_and_link in result_dict.items():
        print 'Result of %s : %s' % (website, URLDICT[website])
        print '-' * 40
...
def exportToHTML(result_dict):
    ...
    for website, title_and_link in result_dict.items():
        ...
        for title,link in title_and_link.items():
            link_line = "<tr><td>" + "<a href=\"%s\">%s</a>" % (link, title) + "</td></tr>"
